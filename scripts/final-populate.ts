...
async function createSources(): Promise<void> {
  ...
  const sources = [
    ...
    },
    {
      ...
      description: 'Portal do CJF com enunciados'
    }
  ];
  ...
  for (const source of sources) {
    ...
    if (error) {
      ...
      console.log(`✅ Fonte ${source.name} criada com ID: ${data.id}`);
      // Armazenar IDs para uso posterior
      if (source.name.includes('Planalto')) {
        (global as any).planaltoSourceId = data.id;
      } else if (source.name.includes('STF')) {
        (global as any).stfSourceId = data.id;
      } else if (source.name.includes('STJ')) {
        (global as any).stjSourceId = data.id;
      } else if (source.name.includes('CJF')) {
...
async function createMainLaws(): Promise<void> {
  ...
  const laws = [
    {
      source_id: (global as any).planaltoSourceId,
      ref: 'CF/88',
